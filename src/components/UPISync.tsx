...
export default function UPISync({ summary, onRefresh }: UPISyncProps) {
  ...
  const [editMap, setEditMap] = useState<Record<string, { title: string; category: string; amount: string; date: string }>>({});
  ...
  const handleImportSelected = async () => {
    ...
    // Prepare batch imports array matching server's expected { id, title, amount, category, date, upiRef }
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {upiTxs.map((tx) => {
              ...
              const fields = editMap[tx.id] || { title: tx.title, category: tx.category, amount: tx.amount.toString(), date: tx.date };

              return (
                <div
                  key={tx.id}
                  className={`p-4 rounded-2xl border transition-all flex flex-col md:flex-row md:items-center gap-4 ${isSelected ? "border-[#27ae60] bg-emerald-50/20 dark:bg-slate-800/30" : "border-[#e2e8f0] dark:border-[#334155]"}`}
                >
                  {/* Select Checkbox */}
                  ...
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-4 gap-3">
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Merchant / Title</label>
                      <input
                        type="text"
                        value={fields.title}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-1.5 bg-white border border-[#e2e8f0] dark:bg-[#1e293b] dark:border-[#334155] rounded-lg text-xs font-bold focus:outline-none focus:ring-1 focus:ring-[#27ae60] dark:text-white"
                      />
                    </div>
                  </div>
